...
function submitForm(event) {
  ...
  Object.keys(values).forEach(k => { if (values[k] === '') delete values[k]; else if (['budget_min','budget_max','year','mileage_km','asking_price','estimated_extra_costs','purchase_price','sale_price','total_costs','service_fee','profit_amount','projected_profit_amount'].includes(k)) values[k] = Number(values[k]); });
  try {
...
function formSchema(collection) {
  ...
  const importCases = [['status','Status','select',STATUS_OPTIONS.importCases],['contact_id','Klant','selectRef','contacts'],['vehicle_id','Voertuig','selectRef','vehicles'],['seller_partner_id','Verkoper/partner','selectRef','partners'],['title','Titel'],['case_number','Dossiernummer'],['risk_level','Risico','select',['laag','normaal','hoog']],['target_delivery_date','Geplande aflevering','date'],['origin_country','Vanuit land'],['destination_country','Naar land'],['destination_port','Bestemmingshaven'],['export_route','Exportroute'],['purchase_price','Aankoopprijs','number'],['sale_price','Verkoopprijs','number'],['total_costs','Totale kosten','number'],['service_fee','Service fee','number'],['profit_amount','Winst','number'],['profit_booked_at','Winst geboekt op','date'],['projected_profit_amount','Verwachte winst','number'],['compliance_notes','Export/compliance notities','textarea'],['summary','Samenvatting','textarea']];
